                ...
                <div className="py-10 text-center text-slate-400 font-bold bg-white rounded-[40px] border border-slate-100">尚無提領紀錄</div>
                 ...
                 <div key={req.id} className="bg-white p-6 rounded-[32px] shadow-sm border border-slate-100 flex flex-col gap-4 animate-in slide-in-from-bottom-2">
                    ...
                    {(req.status === 'Verified' || req.status === 'Approved') && req.receiptUrl && (
                     ...
                     {req.status === 'Pending' && (
                        <button onClick={() => { const url = prompt('請輸入匯款憑證圖片網址：'); if (url) approveWithdrawal(req.id, url).then(() => window.location.reload()); }} className="w-full py-3 bg-emerald-50 text-emerald-600 rounded-[20px] text-[10px] font-black tracking-widest hover:bg-emerald-100 transition-all uppercase mt-2">
                           📤 上傳憑證並核准 (UPLOAD & APPROVE)
                        </button>
                     )}
                  </div>
              ))}
      ...
      <div className="max-w-md mx-auto px-6 pt-10 flex justify-between items-end">
         ...
         <button 
           onClick={async () => {
             ...
             const { fetchDistSalesLogs } = await import("@/app/actions");
