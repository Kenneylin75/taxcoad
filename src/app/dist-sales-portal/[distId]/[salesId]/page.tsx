...
export default function DistSalesPage() {
  ...
  // Financial Filtering
  const [commYear, setCommYear] = useState(new Date().getFullYear().toString());
  const [commMonth, setCommMonth] = useState((new Date().getMonth() + 1).toString().padStart(2, '0'));
  ...
  const handleSaveBankInfo = async () => {
    const res = await updateDistSalesBankInfo(salesId, bankForm);
    if (res.success) {
      alert("銀行帳戶更新成功！");
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <section className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-white p-6 rounded-[32px] shadow-sm border border-slate-100 relative overflow-hidden group hover:border-emerald-200 transition-all">
          ...
          <p className="text-[10px] font-bold text-emerald-600 mt-2">本月獎金：${(commission?.balance || 0).toLocaleString()}</p>
        </div>
      </section>
                    ...
                    <div>
                       <div className="flex items-center gap-3">
                          ...
                          {app.trialMonths > 0 && app.status === 'Active' && (
                              <span className="bg-gradient-to-r from-amber-400 to-orange-500 text-white px-2 py-0.5 rounded-full text-[9px] font-black ml-2 shadow-sm">免費體驗中{app.trialDaysRemaining && app.trialDaysRemaining > 0 ? ` (剩 ${app.trialDaysRemaining} 天)` : ''}</span>
                          )}
                       </div>
                       ...
                       <p className="text-[10px] font-bold text-slate-500 mt-1">宮廟帳號: {app.account || '未設定'}</p>
                    </div>
                 </div>
              </div>
              <div className="flex flex-col pt-4 border-t border-slate-50 gap-3">
                 <div className="flex justify-between items-start">
                     <div className="flex flex-col gap-1.5">
                         <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                           方案：<span className="text-emerald-600">${app.paymentCycle === 'Yearly' ? ((app.monthlyRent || 3600) * 12 * (1 - (app.appliedDiscountRate || 20) / 100)).toLocaleString() : (app.monthlyRent || 3600).toLocaleString()}</span>/{app.paymentCycle === 'Yearly' ? '年' : '月'} 
                           <span className="ml-1 text-[8px] text-slate-300">(總繳: ${((app.setupFee || 12000) + (app.paymentCycle === 'Yearly' ? ((app.monthlyRent || 3600) * 12 * (1 - (app.appliedDiscountRate || 20) / 100)) : (app.monthlyRent || 3600))).toLocaleString()} 含開辦費)</span>
                         </p>
                     ...
                     >
                       <span>歷史紀錄</span>
                       <span className={`transition-transform duration-300 ${expandedTempleId === app.id ? 'rotate-180' : ''}`}>▼</span>
                     </button>
                 </div>
                                ...
                                {filtered.map((r: any, idx: number) => (
                                    <div key={idx} className="flex justify-between items-center bg-white p-3 rounded-xl border border-slate-100">
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            {r.receiptUrl && (
                                                <button onClick={(e) => { e.stopPropagation(); setViewingReceiptUrl(r.receiptUrl); }} className="text-[9px] bg-blue-50 text-blue-600 px-2 py-1 rounded font-black hover:bg-blue-100">檢視憑證</button>
                                            )}
                                            <div className={`w-1.5 h-1.5 rounded-full ${r.status === 'Paid' ? 'bg-emerald-500' : r.status === 'Pending' ? 'bg-amber-500' : 'bg-rose-500'}`}></div>
                                        </div>
                                    </div>
                                ))}
                            </div>
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500 pb-12">
       ...
       <div className="bg-emerald-950 text-white p-8 rounded-[48px] shadow-2xl space-y-8 relative overflow-hidden">
          ...
          <div className="flex justify-between items-start relative z-10">
             <div>
                <p className="text-[10px] font-black text-emerald-400 uppercase tracking-[0.3em] mb-2">{commYear}/{commMonth} 總營業額</p>
                <h2 className="text-5xl font-black tracking-tight">${(commission?.totalRevenue || 0).toLocaleString()}</h2>
             </div>
             <button onClick={handleRequestWithdrawal} className="bg-emerald-500 text-white px-6 py-4 rounded-[24px] text-xs font-black shadow-xl shadow-emerald-500/20 active:scale-95 transition-all">申請提領</button>
          </div>
          <div className="grid grid-cols-3 gap-8 pt-8 border-t border-white/5 relative z-10">
             <div className="space-y-1">
                <p className="text-[10px] font-bold text-emerald-500/60 uppercase tracking-widest">本月分潤</p>
                <p className="text-2xl font-black text-emerald-50">${(commission?.totalEarned || 0).toLocaleString()}</p>
             </div>
             <div className="space-y-1">
                <p className="text-[10px] font-bold text-emerald-500/60 uppercase tracking-widest">總可用餘額</p>
                <p className="text-2xl font-black text-emerald-50">${(commission?.balance || 0).toLocaleString()}</p>
             </div>
             <div className="space-y-1">
                <p className="text-[10px] font-bold text-emerald-500/60 uppercase tracking-widest">已提領總額</p>
                <p className="text-2xl font-black text-emerald-500">${(commission?.totalWithdrawn || 0).toLocaleString()}</p>
             </div>
          </div>
       </div>
