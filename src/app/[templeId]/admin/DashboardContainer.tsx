        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-3">
            ...
            <h1 className="text-4xl md:text-5xl font-black text-white tracking-tight leading-tight">
              {greeting}，{templeName ? `"${templeName}"` : ''}<br className="hidden md:block" />歡迎回到營運主控台
            </h1>
        ...
        <div className="bg-white/90 backdrop-blur-xl rounded-2xl border border-white/50 p-5 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-lg transition-all duration-300 group flex flex-col justify-between">
          ...
          <div>
            <div className="flex items-baseline justify-between mb-2">
              ...
              <p className="text-xs font-bold text-slate-700">{storage?.isVip ? `${(storage?.used || 0).toFixed(1)} GB / 無限使用` : `${(storage?.used || 0).toFixed(1)} / ${storage?.total || 100} GB`}</p>
            </div>
            {!storage?.isVip && (
