                                 ...
                                 {group.files.map((file) => (
                                    ...
                                    <div className="px-2 text-center opacity-70 group-hover/item:opacity-100 transition-opacity">
                                       <div className="flex items-center justify-center gap-1 min-w-0">
                                            {file.uploadedBy === 'Member' || file.uploadedBy === 'Guest' ? <span title="信眾上傳" className="flex-shrink-0">👤</span> : <span title="宮廟上傳" className="flex-shrink-0">🏛️</span>}
                                            <p className="text-xs font-bold text-slate-600 truncate min-w-0" title={file.name || '未知檔案'}>
                                                {file.name || '未知檔案'}
                                            </p>
                                        </div>
                                    </div>
            ...
            <form onSubmit={handleCreateGuest} className="w-full max-w-3xl bg-white rounded-[65px] shadow-3xl overflow-hidden border-4 border-slate-900">
              ...
              <div className="p-16 space-y-10 max-h-[75vh] overflow-y-auto scrollbar-hide">
                 ...
                 <div className="grid grid-cols-2 gap-10">
                   ...
                   <div className="space-y-4"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-6">預設登入密碼 (Password)</label><input name="password" type="text" placeholder="建立時請設定密碼" defaultValue={selectedGuest?.password} className="w-full bg-slate-50 border-2 border-slate-100 rounded-[30px] px-10 py-6 text-xl font-black outline-none focus:border-indigo-600 shadow-inner" /></div>
                 </div>
