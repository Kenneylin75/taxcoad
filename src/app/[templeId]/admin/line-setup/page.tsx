...
export default function LineSetupPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchServiceSettings(), fetchServiceDefinitions(), fetchLampCategories(), fetchEvents(), fetchQueueEvents(), getLineConfig(templeId)]).then(([setts, servs, lamps, evts, queues, lConfig]) => {
      setLineConfig(lConfig);
      const allServices = [
                    ...
                    <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
                       <div className="flex items-center justify-between">
                         ...
                         <div className="flex items-center gap-3 bg-slate-50 px-4 py-2 rounded-xl border border-slate-200">
                           <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest">全局推播總開關</span>
                           <button 
                             onClick={() => setLineConfig({...lineConfig, linePushEnabled: !lineConfig.linePushEnabled})} 
                             className={`w-12 h-6 rounded-full flex items-center transition-all p-1 ${lineConfig.linePushEnabled ? 'bg-amber-500' : 'bg-slate-300'}`}
                           >
                             <div className={`w-4 h-4 bg-white rounded-full transition-all shadow-sm ${lineConfig.linePushEnabled ? 'translate-x-6' : 'translate-x-0'}`}></div>
                           </button>
                         </div>
                       </div>
                       ...
                       <div className="space-y-6">
                          <div className="space-y-2">
                             ...
                             <input value={lineConfig.lineChannelToken} onChange={e => setLineConfig({...lineConfig, lineChannelToken: e.target.value})} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl font-mono text-xs focus:border-amber-500 outline-none shadow-inner" placeholder="貼上您的官方 Token 字串..." />
                          </div>
                          <div className="space-y-2">
                             <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Channel Secret</label>
                             <input value={lineConfig.lineChannelSecret} onChange={e => setLineConfig({...lineConfig, lineChannelSecret: e.target.value})} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl font-mono text-xs focus:border-amber-500 outline-none shadow-inner" placeholder="貼上您的 Secret 字串..." />
                          </div>
                          <div className="space-y-2">
                             <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">LINE Login Channel ID</label>
                             <input value={lineConfig.lineLoginClientId} onChange={e => setLineConfig({...lineConfig, lineLoginClientId: e.target.value})} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl font-mono text-xs focus:border-amber-500 outline-none shadow-inner" placeholder="貼上 LINE Login 的 Client ID..." />
                          </div>
                          <div className="p-6 bg-slate-50 rounded-2xl text-slate-800 space-y-4 relative overflow-hidden group/webhook border border-slate-200 shadow-sm">
                             <div className="space-y-1 relative z-10">
                                <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest">系統專屬 Webhook Endpoint</label>
                                <p className="text-[8px] text-slate-400 font-black uppercase tracking-widest italic">* 請將此網址填入 Messaging API 控制台的 Webhook 欄位並開啟 "Use Webhook"</p>
                             </div>
                             <div className="flex flex-col md:flex-row gap-3 relative z-10">
                                <code className="bg-white px-4 py-2 rounded-lg border border-slate-200 text-slate-700 font-black flex-1 text-xs truncate shadow-inner tracking-tight">{window.location.origin}/api/line/webhook?templeId={window.location.pathname.split('/')[1]}</code>
                                <button onClick={() => {navigator.clipboard.writeText(`${window.location.origin}/api/line/webhook?templeId=${window.location.pathname.split('/')[1]}`); alert('網址已複製！')}} className="px-4 py-2 bg-[#06C755] text-white font-black rounded-lg text-xs hover:bg-[#05b34c] transition-all uppercase tracking-widest shrink-0 shadow-sm">複製</button>
                             </div>
                             <div className="space-y-1 relative z-10 mt-4 border-t border-slate-200 pt-4">
                                <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest">LINE Login Callback URL</label>
                                <p className="text-[8px] text-slate-400 font-black uppercase tracking-widest italic">* 請將此網址填入 LINE Login 頻道的 Callback URL 欄位</p>
                             </div>
                             <div className="flex flex-col md:flex-row gap-3 relative z-10">
                                <code className="bg-white px-4 py-2 rounded-lg border border-slate-200 text-slate-700 font-black flex-1 text-xs truncate shadow-inner tracking-tight">{window.location.origin}/api/line/callback?templeId={window.location.pathname.split('/')[1]}</code>
                                <button onClick={() => {navigator.clipboard.writeText(`${window.location.origin}/api/line/callback?templeId=${window.location.pathname.split('/')[1]}`); alert('網址已複製！')}} className="px-4 py-2 bg-[#06C755] text-white font-black rounded-lg text-xs hover:bg-[#05b34c] transition-all uppercase tracking-widest shrink-0 shadow-sm">複製</button>
                             </div>
                          </div>
                       </div>
                    </div>
