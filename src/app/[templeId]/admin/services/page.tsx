...
const AdminMobileView = ({ services, forms, staffList, availableSlots, loadData, activeTab, setActiveTab }: any) => {
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col font-sans">
       ...
       <main className="flex-1 p-4">
          {activeTab === 'services' && services.map(s => (
             <div key={s.id} onClick={() => setEditingService(s)} className="bg-white p-5 mb-4 rounded-3xl border border-slate-100 shadow-sm">
                <h3 className="text-lg font-bold text-slate-900 mb-1">{s.name}</h3>
                <p className="text-[11px] text-slate-400">{forms.find(f => f.id === s.linkedFormId)?.name || '未關聯表單'}</p>
             </div>
          ))}
...
const AdminDesktopView = ({ services, forms, printTemplates, staffList, availableSlots, loadData, activeTab, setActiveTab }: any) => {
  const [editingService, setEditingService] = useState<any>(null);
