                                  ...
                                  <button 
                                    onClick={async () => {
                                      if (confirm('確定要永久刪除此超級業務嗎？其旗下的宮廟及經銷商將自動轉移至超級管理員旗下！')) {
                                        const { deleteSuperSalesAllData } = await import('@/app/actions');
                                        const res = await deleteSuperSalesAllData(acc.id);
                                        if (res.success) {
                                          alert('刪除並轉移成功');
                                          window.location.reload();
                                        } else {
                                  ...
                                  <button 
                                    onClick={async () => {
                                      if (confirm('確定要永久刪除此經銷商及其所有業務嗎？其旗下的宮廟將自動轉移至超級管理員旗下！')) {
                                        const { deleteDistributorAllData } = await import('@/app/actions');
                                        const res = await deleteDistributorAllData(acc.id);
                                        if (res.success) {
                                          alert('刪除並轉移成功');
                                          window.location.reload();
                                        } else {
                 ...
                 <section className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
                    <div className="flex items-center justify-between gap-4 flex-wrap">
                       <div className="flex items-center gap-4">
                          ...
                          <h3 className="text-2xl font-black text-slate-900 italic tracking-tighter uppercase">宮廟營運列表</h3>
                       </div>
                       <div className="flex items-center gap-4 flex-wrap">
                    ...
                    <table className="w-full bg-white rounded-3xl shadow-sm overflow-hidden text-left border-collapse">
                       <thead className="bg-slate-50/80 border-b border-slate-200/80">
                          <tr>
                             <th className="px-6 py-4 text-xs font-black text-slate-500 uppercase tracking-wider w-24">編號</th>
                             <th className="px-6 py-4 text-xs font-black text-slate-500 uppercase tracking-wider w-52">宮廟名稱</th>
                             <th className="px-6 py-4 text-xs font-black text-slate-500 uppercase tracking-wider w-36">登入帳號</th>
                             <th className="px-6 py-4 text-xs font-black text-slate-500 uppercase tracking-wider w-64">直屬歸屬</th>
                             <th className="px-6 py-4 text-xs font-black text-slate-500 uppercase tracking-wider w-48">營運狀態</th>
                             <th className="px-6 py-4 text-xs font-black text-slate-500 uppercase tracking-wider text-right">管理操作</th>
                          </tr>
                       </thead>
                       <tbody className="divide-y divide-slate-100">
                          {pagedTemples.map((acc: any, idx: number) => (
                            <tr key={acc.id} className="hover:bg-slate-50/70 transition-all group">
                               <td className="px-6 py-4.5">
                                  <span className="font-mono text-xs font-black text-slate-500 bg-slate-100/90 px-2.5 py-1 rounded-lg border border-slate-200/70 shadow-2xs">
                                     No.{String((templePage - 1) * accountPageSize + idx + 1).padStart(4, '0')}
                                  </span>
                               </td>
                               <td className="px-6 py-4.5">
                                  <div className="flex flex-col">
                                     <span className="text-sm font-black text-slate-900 leading-snug">{acc.name || acc.templeName || '宮廟管理員'}</span>
                                     <span className="text-[11px] font-bold text-slate-400 mt-0.5 flex items-center gap-1">
                                        ...
                                        {acc.city || acc.region || '台灣'} · {acc.createdAt ? new Date(acc.createdAt).toISOString().split('T')[0] : '建檔中'}
                                     </span>
                                  </div>
                               </td>
                               <td className="px-6 py-4.5">
                                  <span className="font-mono text-xs font-bold text-indigo-900 bg-indigo-50/80 px-2.5 py-1 rounded-md border border-indigo-100/80 tracking-wide uppercase">
                                     {acc.account || `USR-${acc.id}`}
                                  </span>
                               </td>
                               <td className="px-6 py-4.5">
                                  <div className="flex flex-col gap-1">
                                     {(() => {
                                        if (acc.superSalesId && acc.superSalesId !== 'system-hq') {
                                           ...
                                           const superSalesName = superSales?.name || acc.creatorInfo?.salesName || acc.superSalesId;
                                           return (
                                              <>
                                                 <div className="flex items-center gap-1.5 text-[11px] font-bold text-slate-600 bg-slate-100/80 px-2.5 py-0.5 rounded-md border border-slate-200/70 w-fit">
                                                    <i className="fas fa-building text-slate-400 text-[10px]"></i>
                                                    <span>系統總部 HQ</span>
                                                 </div>
                                                 <div className="flex items-center gap-1.5 text-[11px] font-bold text-amber-700 bg-amber-50 px-2.5 py-0.5 rounded-md border border-amber-200/80 w-fit ml-2">
                                                    <span className="text-slate-300 font-mono">↳</span>
                                                    <i className="fas fa-user-tie text-amber-500 text-[10px]"></i>
                                                    <span>超級業務：{superSalesName}</span>
                                                 </div>
                                              </>
                                           );
                                        }
                                        if (acc.distributorId && acc.distributorId !== 'system-hq') {
                                           ...
                                           const sales = acc.salesId && acc.salesId !== 'system-hq' ? initialAccounts.find((a: any) => a.id === acc.salesId) : null;
                                           const salesName = sales?.name || acc.creatorInfo?.salesName || acc.salesId;
                                           return (
                                              <>
                                                 <div className="flex items-center gap-1.5 text-[11px] font-bold text-indigo-700 bg-indigo-50 px-2.5 py-0.5 rounded-md border border-indigo-200/80 w-fit">
                                                    <i className="fas fa-store text-indigo-400 text-[10px]"></i>
                                                    <span>經銷商：{distName}</span>
                                                 </div>
                                                 {salesName ? (
                                                    <div className="flex items-center gap-1.5 text-[11px] font-bold text-emerald-700 bg-emerald-50 px-2.5 py-0.5 rounded-md border border-emerald-200/80 w-fit ml-2">
                                                       <span className="text-slate-300 font-mono">↳</span>
                                                       <i className="fas fa-id-badge text-emerald-500 text-[10px]"></i>
                                                       <span>經銷業務：{salesName}</span>
                                                    </div>
                                                 ) : (
                                                    <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-400 ml-2">
                                                       <span className="text-slate-300 font-mono">↳</span>
                                                       <span>經銷商直營</span>
                                                    </div>
                                                 )}
                               ...
                               <td className="px-6 py-4.5">
                                  <div className="flex flex-col gap-1.5 items-start">
                                     <button 
                                        ...
                                        className={`px-3 py-1 rounded-full text-[11px] font-black transition-all flex items-center gap-1.5 shadow-2xs cursor-pointer ${(!acc.status || acc.status==='Active') ? 'bg-emerald-50 text-emerald-700 border border-emerald-200 hover:bg-rose-50 hover:text-rose-600 hover:border-rose-200' : 'bg-rose-50 text-rose-600 border border-rose-200 hover:bg-emerald-50 hover:text-emerald-700 hover:border-emerald-200'}`}
                                        title="點擊切換帳號啟用/停權狀態"
                                     >
                                        <span className={`w-2 h-2 rounded-full ${(!acc.status || acc.status==='Active') ? 'bg-emerald-500 animate-pulse' : 'bg-rose-500'}`}></span>
                                        {(!acc.status || acc.status==='Active') ? '啟用中 (Active)' : '已停權 (Inactive)'}
                                     </button>
                                     ...
                                     {(() => {
                                        if (acc.freeType === 'Permanent' || acc.planName === '永久免費') {
                                           return <span className="px-2.5 py-0.5 bg-fuchsia-50 text-fuchsia-700 border border-fuchsia-200 rounded-full text-[10px] font-black tracking-wider shadow-2xs">⭐ 永久免費帳戶</span>;
                                        }
                                        if (acc.freeType === 'Trial' && (acc.trialMonths || 0) > 0) {
                                           ...
                                           if (diffDays > 0) {
                                              return <span className="px-2.5 py-0.5 bg-amber-50 text-amber-700 border border-amber-200 rounded-full text-[10px] font-black tracking-wider shadow-2xs">🎁 剩餘 {diffDays} 天試用</span>;
                                           } else {
                                              return <span className="px-2.5 py-0.5 bg-rose-50 text-rose-700 border border-rose-200 rounded-full text-[10px] font-black tracking-wider shadow-2xs">⚠️ 試用已到期</span>;
                                           }
                                        }
                                        return <span className="px-2.5 py-0.5 bg-blue-50 text-blue-700 border border-blue-200 rounded-full text-[10px] font-black tracking-wider shadow-2xs">💼 正式商務方案</span>;
                                     })()}
                                  </div>
                               </td>
                               <td className="px-6 py-4.5 text-right">
                                  <div className="flex items-center justify-end gap-2">
                                     ...
                                     }} className="h-8 px-3.5 bg-gradient-to-r from-amber-400 to-amber-500 text-slate-900 rounded-xl text-xs font-black hover:from-amber-500 hover:to-amber-600 hover:text-white transition-all shadow-xs flex items-center gap-1.5 active:scale-95">
                                        <i className="fas fa-sign-in-alt text-[11px]"></i> 進入後台
                                     </button>
                                     <button onClick={() => setViewingAccountDetail(acc)} className="h-8 px-3 bg-slate-100 text-slate-700 border border-slate-200/80 rounded-xl text-xs font-bold hover:bg-slate-800 hover:text-white hover:border-slate-800 transition-all shadow-xs flex items-center gap-1.5 active:scale-95">
                                        <i className="fas fa-eye text-[11px]"></i> 詳情
                                     </button>
                                     <button 
                                       onClick={async () => {
                                    ...
                                    className="h-8 px-3 bg-rose-50 text-rose-600 border border-rose-200/80 rounded-xl text-xs font-bold hover:bg-rose-500 hover:text-white hover:border-rose-500 transition-all shadow-xs flex items-center gap-1.5 active:scale-95">
                                        <i className="fas fa-trash-alt text-[11px]"></i> 刪除
                                     </button>
                                  </div>
                               </td>
                            </tr>
                          ...
                          <div className="flex items-center gap-10">
                             ...
                             <div className="space-y-2">
                                ...
                                <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">
                                    申請身份：{app.creatorRole?.toLowerCase() === 'supersales' || app.creatorRole?.toLowerCase() === 'super-sales' ? `超級業務員 ${app.creatorId || ''}` : app.creatorRole?.toLowerCase() === 'distributor' ? `經銷商 ${app.creatorId || ''}` : app.creatorRole?.toLowerCase() === 'system' || app.creatorRole?.toLowerCase() === 'super-admin' ? '超級管理員' : (app.creatorRole?.toLowerCase() === 'distsales' || app.creatorRole?.toLowerCase() === 'dist-sales' ? '經銷業務 ' + (app.creatorId || '') : (app.creatorRole ? app.creatorRole + ' ' + (app.creatorId || '') : '未知身份'))} | 
                                    提交：{app.creatorId || '超級管理員'} | 
                                 ...
                                 <div className="flex gap-4 text-xs font-medium text-slate-500 mt-2">
                                    <span className="bg-slate-100 px-2 py-1 rounded">方案：{app.freeType === 'Trial' ? `免費試用 ${app.trialMonths || 0} 個月` : app.freeType === 'Permanent' ? '永久免費' : '標準方案'}</span>
                           ...
                           <div className="flex items-center gap-10">
                              ...
                              <div className="space-y-2">
                                 ...
                                 <div className="flex gap-4 text-xs font-medium text-slate-500 mt-2">
                                    ...
                                    <span className="bg-slate-100 px-2 py-1 rounded">費用：NT$ {Number(app.price || 0).toLocaleString()}</span>
                                    <span className="bg-slate-100 px-2 py-1 rounded">額度：{app.nodes || 100} 間宮廟</span>
                                 </div>
                                 <div className="flex gap-4 text-[11px] font-medium text-slate-400 mt-1">
                                    <span>負責人：{app.contactName || app.owner || '未提供'}</span>
                                          ...
                                          <tr key={b.id} className="hover:bg-slate-50/30 transition-all">
                                             ...
                                             <td className="px-8 py-6 text-right">
                                                <div className="flex items-center justify-end gap-2">
                                                   ...
                                                   {b.receiptUrl && (
                                                      <button 
                                                         onClick={() => setStoragePreviewImage(b.receiptUrl)}
      ...
      {storagePreviewImage && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div 
            ...
            onClick={() => setStoragePreviewImage(null)}
          />
          <div className="relative bg-white rounded-3xl overflow-hidden shadow-2xl max-w-4xl max-h-[90vh] flex flex-col z-10 animate-in fade-in zoom-in duration-200">
            ...
            <div className="overflow-auto p-4 flex items-center justify-center bg-slate-50">
              <img src={storagePreviewImage} alt="匯款圖片" className="max-w-full max-h-[70vh] object-contain rounded-xl" />
            </div>
          </div>
        </div>
      )}
