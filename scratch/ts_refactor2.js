...
function processSqlText(text, quoteType) {
  ...
  // Helper to format table/column names depending on JS quotes
  const q = (str) => {
    if (quoteType === '"') return `\\\\"${str}\\\\"`; // Escapes to \" in the file
    return `"${str}"`; // Just " in single quotes or backticks
  };
...
project.getSourceFiles().forEach(sourceFile => {
  ...
  const allNodes = [
    ...
    ...sourceFile.getDescendantsOfKind(SyntaxKind.TemplateExpression) // Wait, template expression getText() includes ${} which is not ideal, but we can't easily parse without it. We'll replace over the whole string.
  ];
