...
const oldBlock = `                             <div key={idx} className="bg-slate-50/50 p-8 rounded-[32px] border border-slate-100 flex gap-6 items-end group hover:bg-white hover:shadow-xl hover:border-indigo-100 transition-all relative">
...
const newBlock = `                             <div key={idx} className="bg-slate-50/50 p-8 rounded-[32px] border border-slate-100 flex flex-col gap-4 group hover:bg-white hover:shadow-xl hover:border-indigo-100 transition-all relative">
                                ...
                                {field.type === 'select' && (
                                   <div className="ml-4 space-y-2 animate-in fade-in slide-in-from-top-2 duration-300">
                                      ...
                                      <input value={(field.options || []).join(', ')} onChange={e => {
                                         ...
                                         setEditingForm({...editingForm, fields: next});
                                      }} className="w-full bg-white border border-slate-100 rounded-xl px-4 py-3 text-sm font-bold outline-none focus:border-indigo-600 transition-all text-indigo-600 placeholder:text-slate-300" placeholder="例如：柏油路, 水泥路, 石頭路" />
                                   </div>
                                )}
                             </div>`;
...
console.log('Options editor added to form builder.');
